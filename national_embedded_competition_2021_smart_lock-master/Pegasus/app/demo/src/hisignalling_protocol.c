...
hisignalling_error_type hisignalling_msg_receive(hi_u8 *buf, hi_u32 len)
{
    ...
    // static unsigned int czx = 0;
    // static unsigned int szy = 0;
    // static unsigned int zzy = 0;
    // static unsigned int stranger = 0;
    ...
    // char str_buf[4] = {"\0"};
    // str_buf[0] = buf[HISIGNALLING_MSG_PAYLOAD_INDEX] / 100 + '0';
    // str_buf[1] = buf[HISIGNALLING_MSG_PAYLOAD_INDEX] / 10 % 10 + '0';
    // str_buf[2] = buf[HISIGNALLING_MSG_PAYLOAD_INDEX] % 10 + '0';

    // oled_fill_screen(OLED_CLEAN_SCREEN);
    // oled_show_str(45, 2, str_buf, 1);
